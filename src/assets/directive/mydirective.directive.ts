import { HostListener, ElementRef, Input, Output, EventEmitter, Directive, OnInit, Inject, LOCALE_ID } from '@angular/core';
...
export class finDateDirective {
  ...
  @HostListener('focusout', ['$event']) onLoad(value) {
    ...
    var sdt = formatDate(this.provider.companyinfo.company.from_date, 'yyyy/MM/dd', 'en-US');
    var edt = formatDate(this.provider.companyinfo.company.to_date, 'yyyy/MM/dd', 'en-US');

    if (this.customdate >= sdt && this.customdate <= edt) {
