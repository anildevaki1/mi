...
@Component({
  selector: 'app-mfgr',
  templateUrl: './mfgr.component.html',
  styleUrls: ['./mfgr.component.scss']
})
export class MfgrComponent implements OnInit {
  entity: any = {}
  reference: any = {};
  mode: boolean = false;
  list: any = [];
  ...
  showRecord() {
    ...
    this.spinner.show();
    this.http.get('Finance/gettradeAndMfgr', params).subscribe({
      next: (response: any) => {
        ...
        this.provider.ShareData.audit.TradeAndMfgr = {};
        this.provider.ShareData.audit.TradeAndMfgr.data = data;
        this.provider.ShareData.audit.TradeAndMfgr.tdate = this.entity.tdate;
        this.list = this.provider.ShareData.audit.TradeAndMfgr.data;
     this.spinner.hide();
  ...
  printReport() {
    if (this.entity.reportType != undefined) {
      this.entity.url = "Reports/Finance/Audit/Pages/tradeAndMfgr";
      var companyinfo = this.provider.companyinfo.selectedbranch;
      this.entity.params = [
